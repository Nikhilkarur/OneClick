...
- `/health` returns 503 until the catalog, Screen Graph, vector index and cache snapshot are loaded (`app/obs/readiness.py`). The no-SIIS lookup table (`cache/no_siis.py`, formerly `cache/lookup.py`) is still a stub, so a request without an article returns empty `contexts` with `fallback: no_siis_context`.
